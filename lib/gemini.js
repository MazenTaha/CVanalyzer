...
function getGeminiApiKey() {
  ...
  // Safe debug log: this confirms whether the server can see the variable
  // without printing the secret value itself.
  console.log(`[Gemini] GEMINI_API_KEY configured: ${apiKey ? "yes" : "no"}`);
...
Analyze the following CV or LinkedIn summary.
Return ONLY valid JSON.
Do not include markdown.
Evaluate clarity, professionalism, technical strength, ATS readiness, missing skills, and interview readiness.
...
function parseGeminiResponse(rawText) {
  ...
  try {
    ...
    return normalizeAnalysis(JSON.parse(extractJsonObject(cleanedText)));
  }
}

export async function analyzeProfileText(text) {
